import type { ContentType, DensityLevel, InputShape, MarkdownAnalysis, PlanningProfile, PreprocessedMarkdown, RewriteStrategy, RoughnessLevel } from './types.js';
import { preprocessMarkdown } from './preprocess.js';

// 轻量 Analyzer：先识别输入形态，再决定 Planner 应该保守还是激进重写。
const detectContentType = (source: PreprocessedMarkdown): ContentType => {
  const joined = [source.deck_title, ...source.sections.map((section) => [section.title, ...section.points].join(' '))].join(' ').toLowerCase();
  if (/(课程|教学|本节|学习|课时)/.test(joined)) return 'course';
  if (/(汇报|结论|建议|指标|分析)/.test(joined)) return 'report';
  if (/(故事|经历|转折|结局)/.test(joined)) return 'story';
  if (/(产品|方案|价值|优势|客户|增长)/.test(joined)) return 'pitch';
  return 'general';
};
...
const detectRoughness = (source: PreprocessedMarkdown, markdown: string, inputShape: InputShape, density: DensityLevel): RoughnessLevel => {
  ...
  if (explicitSections === 0 && explicitBullets <= 2 && hasLooseSentenceStyle) return 'very_rough';
  if (inputShape === 'notes_like' || genericTitles >= 2 || avgPointLength >= 40) return 'very_rough';
  if (inputShape === 'document_like' || density === 'high' || avgPointLength >= 26) return 'rough';
  return 'clean';
};
...
const suggestSlideCount = (source: PreprocessedMarkdown, density: DensityLevel): number => {
  const base = Math.max(4, Math.min(10, source.sections.length || 4));
  if (density === 'high') return Math.min(10, base + 2);
  if (density === 'low') return Math.max(4, base - 1);
  return base;
};

// 不同内容类型走不同规划策略，让 Planner 真正分流。
export const buildPlanningProfile = (analysis: MarkdownAnalysis): PlanningProfile => {
  const commonRules = [
    '一页只讲一个核心点',
    '标题使用上屏短标题，不复述原文小节名',
...
export const analyzeMarkdown = (markdown: string): MarkdownAnalysis => {
  ...
  if (inputShape === 'document_like') notes.push('contains longer sections that should be compressed into slide points');
  if (inputShape === 'notes_like') notes.push('contains rough notes that may need title normalization');
  if (density === 'high') notes.push('content density is high; split overloaded sections rather than keeping long bullets');
  if (roughness === 'rough') notes.push('rewrite slide titles and shorten explanatory sentences before presenting them');
  if (roughness === 'very_rough') notes.push('treat this as draft material: infer structure, fill missing transitions, and rewrite aggressively');
  if (contentType === 'course') notes.push('preserve teaching sequence and end with a concise summary slide');
  if (contentType === 'report') notes.push('keep conclusion-oriented titles and analytical structure');
  if (contentType === 'pitch') notes.push('surface the problem-value-solution arc instead of mirroring the original wording');
