...
export const buildHeuristicOutline = (markdown: string): OutlineResult => {
  ...
  const slides = source.sections.length
    ? source.sections.map((section, index) => ({
        ...
        intent: (index === 0 ? 'define' : 'explain') as SlideIntent
      }))
    : [{ index: 1, title: source.deck_title, summary: '内容概览', preview_points: ['内容概览'], detail_points: ['内容概览'], intent: 'summary' as const }];
...
export const buildHeuristicExpanded = (markdown: string, outline: OutlineResult): ExpandedResult => {
  const source = preprocessMarkdown(markdown);
  ...
  return {
    ...
    slides: outline.slides
      .map((slide, index) => {
        ...
        const format = ((slide.intent === 'summary' || bullets.length <= 1) ? 'title-body' : 'title-bullets') as ExpandFormat;
