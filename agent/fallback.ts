...
function toSlideTitle(text: string, fallback: string): string {
  const cleaned = compactClause(
    text
      .replace(/^#+\s*/, '')
...
const looksPlanningMeta = (value: string): boolean => /^(一些想法|后面也许可以举例|也许可以举例|可以举例|还要提一下|可能还要提一下|我想做一个|第一页讲)/.test(value);
...
function sentenceUnits(section: SourceSection): string[] {
  const pool = [...section.points].map((item) => item.trim()).filter(Boolean);
  const units = pool.flatMap((item) =>
    item
      .split(/[。！？；;\n]/)
      .flatMap((part) => part.split(/[，,:：]/))
...
function splitBuckets(units: string[], roughness: MarkdownAnalysis['roughness']): string[][] {
...
function regroupSections(
  source: ReturnType<typeof preprocessMarkdown>,
...
function buildPreview(points: string[]): string[] {
  return points.slice(0, 3).map((item) => truncate(compactClause(item), 16));
}

function buildOmittedTopics(source: ReturnType<typeof preprocessMarkdown>, slides: Array<{ title: string; points: string[] }>, analysis: MarkdownAnalysis): string[] {
  if (analysis.roughness === 'clean' && analysis.density !== 'high') return [];
...
function inferCoreMessage(source: ReturnType<typeof preprocessMarkdown>, slides: Array<{ title: string; points: string[] }>, deckTitle: string): string {
  const candidates = [
...
function inferIntent(title: string): SlideIntent {
  const value = title.toLowerCase();
  if (value.includes('总结') || value.includes('结论')) return 'summary';
  if (value.includes('案例') || value.includes('示例')) return 'example';
  if (value.includes('区别') || value.includes('对比')) return 'compare';
  if (value.includes('步骤') || value.includes('流程') || value.includes('如何')) return 'process';
  if (value.includes('定义') || value.includes('什么是')) return 'define';
  return 'explain';
}
...
function buildSlideBody(slide: OutlineSlide): string {
  ...
  if (points.length === 0) return slide.summary ?? '';
